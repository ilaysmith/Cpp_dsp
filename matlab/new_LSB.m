clc ; clear all; close all

%% Чтение файла 
[y,Fs] = audioread('/home/morda/task_for_stc_1/Cpp_dsp/sound/7.133400_02.05.2023 14_57_07_LSB.iqw');

%% Формирование огибающей
%signal = y(1:2:end).'; % SSB сигнал состоит либо из I либо из Q
signal = y(:,1); % SSB сигнал состоит либо из I либо из Q - I LSB

% нужно сместить сигнал на -1000 Гц, так как на 0 Гц должна была бы быть
% постоянная составляющая
t = (0:length(signal)-1)'/Fs;
shift = -1000; % Гц
signal_shift = (signal.*exp(1i*2*pi*shift*t));
%% Фильтрация 

z = signal_shift - mean(signal_shift);

%% Нормирование

final = z / max(abs(z));
%% 
stereo_signal = [real(final), imag(final)];  % N x 2 матрица

%% Запись 
audiowrite('/home/morda/task_for_stc_1/Cpp_dsp/sound/LSB_MATLAB_IQW.wav',stereo_signal,Fs);
