...
// Определение метода класса Фильтр для SSB
std::vector<float> filter::delete_const_complex(Signal &signal) {
    Complex<float> sum(0.0, 0.0);   // Для суммирования всех отсчётов
    std::vector<float> result; // Для сигнала без постоянной составляющей + **1
    result.reserve(signal.signal.size()); // выделение памяти как под исходный сигнал

    for (int i = 0; i < signal.signal.size(); i++) {
        sum += signal.signal.at(i); // Сложение всех элементов. at() проверяет выход за границы + **2
    }
    Complex<float> mean = sum * (1.0f / signal.signal.size()); // Вычисление среднего // 1.0f - float

    for (int i = 0; i < signal.signal.size(); i++) {
        Complex<float> sample = signal.signal.at(i); // Берём значение отсчёта

        result.push_back((sample - mean).get_real()); // Вычитаем среднее из каждого отсчёта и записываем real
        result.push_back((sample - mean).get_imag()); // Вычитаем среднее из каждого отсчёта
    }
